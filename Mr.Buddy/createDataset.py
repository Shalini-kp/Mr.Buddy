...
def getWhatsAppData():
    personName = "$h@lin!"  # input('Enter your full WhatsApp name: ')
    ...
    for index, row in combined.iterrows():
        ...
        name = row["From"].replace(" ", "").join(row["From"].split())
        print("From \n", len(name), "personName", len(personName))
        if (name == personName):
            print("*************************")
            if (firstMessage):
                firstMessage = False
            ...
            if myMessage and otherPersonsMessage:
                print("&&&&&&&&&&&&&&&&&&&&&&&&&")
                # print("myMessage\n",myMessage, "otherPersonsMessage", otherPersonsMessage)
